...
static void correctheight(const Arg *arg);
static Client* nextontag(int target);
...
nextontag(int target)
{
  ...
  if (!selmon->sel || (selmon->sel->isfullscreen && lockfullscreen))
    return NULL;
  if (target > 0) {
    for (c = selmon->sel->next; c && (!ISVISIBLE(c) || (target == 1 && HIDDEN(c))); c = c->next);
    if (!c)
      for (c = selmon->clients; c && (!ISVISIBLE(c) || (target == 1 && HIDDEN(c))); c = c->next);
  } else {
    for (i = selmon->clients; i != selmon->sel; i = i->next)
      if (ISVISIBLE(i) && !(target == -1 && HIDDEN(i)))
        c = i;
    if (!c)
      for (; i; i = i->next)
        if (ISVISIBLE(i) && !(target == -1 && HIDDEN(i)))
...
void
correctheight(const Arg *arg) // i for up line cur_w, f for down line cur_w
{
  Client *cur_w;

  if (!(cur_w = selmon->sel) || !ISVISIBLE(cur_w))
        return;

  if (selmon->lt[selmon->sellt]->arrange != tile || !nexttiled(selmon->clients))
    return;

  Client* edit_down_w = nextontag(1);

  Client* edit_up_w = nextontag(-1);

  if (edit_up_w && edit_up_w->x == cur_w->x) {
    resizeclient(edit_up_w, edit_up_w->x, edit_up_w->y,
      edit_up_w->w, edit_up_w->h = edit_up_w->h - arg->i);
  }

  if (edit_down_w || edit_up_w) {
    resizeclient(cur_w, cur_w->x,
      cur_w->y = cur_w->y - arg->i,
      cur_w->w, cur_w->h = cur_w->h + arg->i - (int)arg->f);
  }

  if (edit_down_w && edit_down_w->x == cur_w->x) {
    resizeclient(edit_down_w, edit_down_w->x,
      edit_down_w->y = edit_down_w->y - (int)arg->f,
      edit_down_w->w, edit_down_w->h = edit_down_w->h + (int)arg->f);
    }
